...
- **`make add-method` refuses a bundle path that is a symlink**: a `.mthds` link is no longer read through to its target, as a link inside a bundle directory already was not, and the refusal for either no longer says the rule applies only under `methods/` and `src/generated/`.
