...
- **`make add-method`**: scaffolds a method that lives on the platform (`METHOD=mt_…`) or in a published package (`METHOD=github.com/owner/repo[/pkg][@tag]`) into the app, writing its manifest, generated tree, adapter, Server Action trio, action test, form and registry entry. The action reads its selector from the manifest, so moving to another version of the method is editing `method.json` and running `npm run codegen`.
