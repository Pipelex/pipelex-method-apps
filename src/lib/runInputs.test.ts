...
import { INPUT_FORM, PIPE_IO_CONTRACTS } from "@/test/fixtures/contracts/extract-entities";
import {
  INPUT_FORM as PDF_INPUT_FORM,
  PIPE_IO_CONTRACTS as PDF_CONTRACTS,
} from "@/test/fixtures/contracts/summarize-pdf";
import {
  INPUT_FORM as COMPLEX_INPUT_FORM,
  PIPE_IO_CONTRACTS as COMPLEX_CONTRACTS,
} from "@/test/fixtures/contracts/complex-form";
...
describe("gateRunInputs", () => {
  ...
  it("drops an input the contract does not declare", () => {
    // The gate is the trust boundary, and a file-input action scans exactly what it
 ...
 * below predate the `complex-form` fixture, and they are kept rather than
 * replaced by it: they isolate one shape each, including shapes no recorded
 * contract has (a struct with required children, a half-filled struct). For a
 * `prose` or `document` field — all the other fixtures derive to those —
...
describe("the gate agrees with the Run button", () => {
   ...
   * gates.) The `complex-form` fixture declares one too; this isolates the branch.
   */
  ...
  const cases: Array<{
    ...
    // The complex-form fixture, a recorded contract — the one fixture that
    // reaches the structured and plural branches for real. Every state its form
    // can be in belongs here, because real codegen output is the likeliest way
    // to reintroduce the failure the next row describes.
    {
