...
- **`src/actions/`** — Server Actions (`"use server"`), one file per method once methods exist. The only place that calls the Pipelex SDK. Each method exports a **trio**: `run<Name>Blocking` (the blocking `execute` path), `start<Name>Run` + `poll<Name>Run` (the durable start/poll path). They are thin: gate → build options → delegate to `executeBlockingRun` / `startDurableRun` / `pollDurableRun`.
...
- **`src/test/fixtures/contracts/`** — recorded `contracts.ts` files, real codegen output for methods this app does not ship, so the shared code's tests (`runInputs`, `resultField`, `resultUrls`, `RunResult`) run against the shapes a real method produces. They are test data, never imported by app code.
...
This is the trio `make add-method` writes for a method with no file input, abridged:
...
- **File/image outputs come back as a URL** — a storage URL or a base64 data URL — in the output content. On the hosted durable path the runtime returns both a non-web `url` (`pipelex-storage://…`) and a web `public_url` (a signed S3 URL), and the kernel's file arms prefer `public_url`, which is why a hosted run's files paint without a resolver. **The result view applies a URL policy of its own, and it is the one thing it re-reads.** The form kernel decides what to paint, link and frame from its own `isViewableUrl`, which accepts `http:`, **any** `data:` media type and `blob:` — and one of the sinks behind that verdict is a `DocumentPreview` `<iframe>` with no `sandbox` attribute, offered whenever the payload's own `filename` or `mime_type` looks previewable. So `scrubResultUrls` (`src/lib/resultUrls.ts`) walks the result descriptor exactly as `checkFileInputs` walks the input one, and removes any file URL the kernel would act on that is not `https:` or a PNG/JPEG/WebP `data:` URL, reporting what it removed so `<RunResult>` can say so rather than quietly differ. It normalizes an accepted URL too, so the string this judged is the string the kernel gets. A reference the kernel would never touch (`pipelex-storage://`) is left verbatim. **This is a stopgap owned upstream** — the fixes belong in `@pipelex/mthds-form` — and it does not cover the markdown a `native.Text` result carries, where a `![](https://…)` in the model's own answer loads on paint.
- **A `pipelex-storage://` reference on its own resolves nowhere in a browser**, and the kernel's seam for exchanging one is a `<ResultEnvProvider resolveUrl>` mounted above the result — one provider high in the tree, not a prop threaded through `<RunResult>`. Nothing mounts it yet.
...
- **A `make` variable counts only when it is given on the command line.** The Makefile's `opt` and `require` helpers read a variable's origin before its value, so a `NAME` or `LABEL` the shell exports is not taken as a request, and `shq` hands the value to the script exactly as typed. A new gesture's variables go through the same helpers — never `$(if $(NAME),--name $(NAME))`, which both inherits the environment and lets the shell reinterpret the value.
