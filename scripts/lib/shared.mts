...
/** The app-owned staleness sidecar, written beside each lock. Not stamped, not locked. */
export const SOURCES_SIDECAR = "sources.json";

/**
 * The app-owned manifest that names a method living somewhere other than
...
export const SIDECAR_COMMENT =
  ...
  "is the SHA-256 of each app-emitted artifact the codegen lock does not sign. " +
 ...
 * both halves of it: the `.mthds` sources and the app-emitted artifacts.
 *
 ...
 *
 * A missing `methods/` is no method at all rather than an error: the template
 * ships none, and git keeps no empty directory.
 */
export async function discoverMethods(methodsDir = METHODS_DIR): Promise<MethodSource[]> {
  await refuseSymlinkRoot(methodsDir);
  let entries: Dirent[];
  try {
    entries = await readdir(methodsDir, { withFileTypes: true });
  } catch (error) {
    if (isEnoent(error)) return [];
    throw error;
  }
 ...
 * app-owned `sources.json` sit beside the lock without being called an orphan.
 */
 ...
 * the app-emitted artifacts it stamps.
 *
 ...
 * as a no-verdict: it is app-owned, and regenerating both restores it and
 * re-proves the tree.
 */
