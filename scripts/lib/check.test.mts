...
describe("runCheck over an app with no method", () => {
  // The template ships no method, so this is the state of every fresh clone —
  // and `make all` runs the check, so it has to be green.
  it("is current when there is no methods/ directory and no generated tree", async () => {
    const empty = path.join(generatedRoot, "empty");
    await mkdir(empty);
    const roots = {
      methodsDir: path.join(empty, "methods"),
      generatedRoot: path.join(empty, "src", "generated"),
    };
    expect(await runCheck(roots)).toBe(EXIT_CURRENT);
  });

  it("is current when methods/ holds only loose files", async () => {
    const methodsDir = path.join(generatedRoot, "methods");
    await mkdir(methodsDir);
    await writeFile(path.join(methodsDir, "README.md"), "no method here\n");
    const roots = { methodsDir, generatedRoot: path.join(generatedRoot, "absent") };
    expect(await runCheck(roots)).toBe(EXIT_CURRENT);
  });

  it("still calls a generated tree left behind drift", async () => {
    // Removing the last method without its tree is the one empty state that is
    // not current: the tree describes a method nothing regenerates any more.
    const methodsDir = path.join(generatedRoot, "no-methods");
    const treesDir = path.join(generatedRoot, "trees");
    await mkdir(path.join(treesDir, "leftover"), { recursive: true });
    expect(await runCheck({ methodsDir, generatedRoot: treesDir })).toBe(EXIT_DRIFT);
  });
});
