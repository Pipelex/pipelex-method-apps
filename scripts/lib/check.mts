 ...
 *     and are the app's own artifacts intact?** The lock cannot know either
 ...
 *     (`generate.mts`) puts a app-owned `sources.json` beside each lock and
 ...
 * 2 no verdict could be produced. An app with no method and no leftover tree is
 * current: there is nothing it could be stale against, and the template ships
 * exactly that state, so its `make all` has to be green. Aggregation across methods is by precedence,
...
/** Where the check reads from — the repo's own directories unless a test says otherwise. */
export interface CheckRoots {
  methodsDir: string;
  generatedRoot: string;
}

const REPO_ROOTS: CheckRoots = { methodsDir: METHODS_DIR, generatedRoot: GENERATED_ROOT };

async function runCheckInner(roots: CheckRoots): Promise<number> {
  let methods: MethodSource[];
  try {
    methods = await discoverMethods(roots.methodsDir);
  } catch (error) {
    if (
  ...
  let scan;
  try {
    scan = await findOrphanTrees(roots.generatedRoot, new Set(methods.map((m) => m.name)));
  } catch (error) {
    if (error instanceof SymlinkRefusedError) {
  ...
  if (methods.length === 0 && scan.orphans.length === 0 && scan.caseMismatches.length === 0) {
    console.log(
      `codegen:check: no method under ${path.relative(REPO_ROOT, roots.methodsDir)}/ and no generated tree — nothing to check.`,
    );
    return EXIT_CURRENT;
  }
  ...
  for (const method of methods) {
    codes.push(await checkMethod(method, roots.generatedRoot));
  }
 ...
 */
export async function runCheck(roots: CheckRoots = REPO_ROOTS): Promise<number> {
  try {
    return await runCheckInner(roots);
  } catch (error) {
