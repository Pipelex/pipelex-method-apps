...
describe("discoverMethods over no methods", () => {
  it("finds none when methods/ does not exist, rather than failing", async () => {
    await expect(discoverMethods(path.join(fixtureDir, "no-such-dir"))).resolves.toEqual([]);
  });
});
