...
describe("registerMethod", () => {
  ...
  async function realRegistry(): Promise<string> {
    return readFile(path.join(REPO_ROOT, "src", "methods.ts"), "utf-8");
  }

  it("inserts one import and one entry into the real src/methods.ts", async () => {
    const source = await realRegistry();
    const updated = registerMethod(source, ENTRY);

    expect(updated).toContain('import { DemoForm } from "@/components/DemoForm";');
...
describe("renderAdapter", () => {
  it("hands wireOutput to the method's own generated binder", () => {
    ...
    expect(source).toContain("return parseText(wireOutput(results));");
    expect(source).toContain('import type { Text } from "@/generated/text-stats/types";');
  ...
  it("types a plural output as a list of the concept, read through wireListOutput", () => {
    ...
    expect(source).toContain("DocumentsOutputSchema.parse(wireListOutput(results))");
...
describe("renderActionTest", () => {
  ...
  it("pins the bundle it sends when a bundle-sourced pipe gates on nothing", () => {
    ...
    expect(source).toContain('pipe_code: "receipt_review.review_receipts",');
...
describe("runAddMethod", () => {
  ...
  it("writes the manifest, the tree and the four app files, and edits the registry", async () => {
    ...
    const registry = await readFile(path.join(root, "src/methods.ts"), "utf-8");
    expect(registry).toContain('import { TextStatsForm } from "@/components/TextStatsForm";');
  ...
  it("refuses a registry whose anchor is gone, before writing anything", async () => {
    const registryPath = path.join(root, "src/methods.ts");
