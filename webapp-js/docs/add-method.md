...
A bundle is refused when the path does not exist, when a file is not a `.mthds` file, when a directory holds none, when the path is a symlink or contains one, when it contains a file that is not UTF-8, when the directory contains this app, when it names `methods/` itself or a file sitting directly in it (no method is read from there), and when a method directory in place also holds a `method.json`.
