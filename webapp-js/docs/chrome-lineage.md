...
- **The Makefile.** Besides its comments and help text, which say the keyed targets need the api-dev base URL for the time being, the `add-method` target reads only the non-blank variables given on the `make` command line and passes their values to the script exactly as typed, through the `shq`, `given`, `opt`, `flag` and `require` helpers, which `scripts/lib/makefile.test.mts` pins. The gallery's target tests `METHOD` inside a shell string, where a value holding `$(…)` is executed.
