...
describe("scrubResultUrls", () => {
  ...
  it("runs over a real generated image field", () => {
    // A recorded contract, not a hand-built descriptor: this is the field an
    // image-producing method renders, so a codegen change that moved the node's
    // kind would show up here rather than only in production.
