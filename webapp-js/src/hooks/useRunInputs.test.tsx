...
import { INPUT_FORM, PIPE_IO_CONTRACTS } from "@/test/fixtures/contracts/summarize-pdf";
...
describe("useRunInputs", () => {
  ...
  it("narrows a file input to the media types the upload action grants", () => {
    const { result } = renderHook(() =>
      useRunInputs(CONTRACT, DESCRIPTOR, { allowedMimes: ["application/pdf"] }),
    );
    // The scaffolded action grants a document method PDF alone, so the dropzone
    // must not offer the PNG it would then refuse.
