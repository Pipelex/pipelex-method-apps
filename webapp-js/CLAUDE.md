...
scripts/                      # native-Node TypeScript (node --experimental-strip-types)
  ...
  serve.mts                   # make serve / make stop — CLI entry over lib/serve
  lib/                        # the behavior, importable so it can be tested
    ...
    serve.mts                 # serve + stop — the dev server in the background, proven; needs Node and lsof only
...
src/
  ...
  config.ts                   # ExecutionMode + EXECUTION_MODE, the deployment's choice (client-safe)
  ...
  lib/
    ...
    fileInputs.ts             # checkUploadRequest + checkFileInputs — the grant's type + size gate, the run's scheme gate (pure)
    uploadGrant.ts            # grantFileUpload — requestUploadGrant behind the upload gate (server)
    runRequest.ts             # MAX_RUN_INPUT_BYTES + runInputBytes — the run's input size, checked before the action (pure)
  ...
  hooks/
    ...
    useFileInputs.ts          # drop → grant → upload to storage → write back the reference; the kernel's file seam (client)
  components/
    ...
    RunDetails.tsx            # client component — a finished run's id (selectable, Copy) + its cost, folded
...
src/types/<camel>Uploads.ts                # ALLOWED_MIMES, read by the grant action and the form (only with a file input)
src/actions/run<Pascal>Pipeline.ts         # "use server" — run<Pascal>Blocking + start<Pascal>Run + poll<Pascal>Run (+ request<Pascal>Upload with a file input)
...
- **Regeneration needs a base URL that serves what codegen asks for**: `/v1/validate`'s `input_form` and `output_form` views, both of which codegen needs for **every** method, and, for a manifest, the selector kind it names (`method_ref` for a package, `method_id` for a catalog id). The default, `https://api.pipelex.com`, serves all of it; a base URL pointed elsewhere may not, and both scripts name the missing capability rather than failing obscurely. `codegen:check` needs no server at all, so `make all` stays green offline regardless.
...
**Two execution modes, one hook.** Every method runs in one of two modes, and the deployment chooses which: `EXECUTION_MODE` in `src/config.ts`, read from `NEXT_PUBLIC_EXECUTION_MODE`. The page offers no switch, because a person using the app has no reason to know the difference, and one who picked Blocking would see any method that runs past the gateway's cap fail.

- **Blocking** (`client.execute`) — one synchronous request. Simple, but behind the hosted gateway it is cut off at ~30s, so long pipelines surface a classified `execute_timeout` error. It is the mode for a deployment that does not serve the durable run lifecycle, such as a local open-source runner.
...
The forms are **mode-agnostic** — they call `useRun({ mode: EXECUTION_MODE, blocking, start, poll })` and render by `state.phase`. Only the unified hook knows which Server Actions to call. Both terminal states carry the run's id, in both modes: `<ErrorDisplay>` shows it for a failed run, and `<RunDetails>` shows it under a finished run's result, selectable and with a Copy button, beside a closed "Usage and cost" disclosure holding `<CostReport>`. `executeBlockingRun` logs a blocking run's id when it finishes, as `startDurableRun` logs a durable run's when it starts, so every run is in the server's log, on every server.
...
There is no `pipe_output` search arm: the SDK resolving `.main_stuff` on both paths removed the shape-guessing, and `Schema.parse` rejects arrays, primitives, and `null` with a message naming the offending field. **Reaching an intermediate stuff no longer waits on the SDK:** `RunResults` carries the run's named stuffs as `working_memory` (`{ root, aliases }`) since `@pipelex/sdk` 0.18.0, filled on the durable path. The blocking path has them too: `executeBlockingRun` lifts the execute response with the SDK's `resultsFromExecute`, which carries `working_memory` onto `RunResults` beside `main_stuff` and the usage pair, so a narrower reads an intermediate stuff the same way on both paths.
...
- **The kernel never uploads — the host does.** `DocumentField` fires `env.onDropFile(id, file)` and waits; `useFileInputs` stores the file and writes a `FileValue` (`{url, filename}`) back at the field's **dotted path** with `setValueAtPath`. While the id sits in `env.uploadingIds` the kernel shuts **every door into that value** — the dropzone, the "paste a URL instead" toggle and the URL input behind it — which is why no staleness token is needed for a second selection mid-upload and why the form's `disabled` is plain run state. The one write path that guarantee cannot cover is the host's own chrome: a shortcut button a host adds to fill the field must disable itself while the field is resolving (`running || uploadingIds.size > 0`).
- **A dropped file goes straight from the browser to Pipelex storage, and a run carries references only.** `useFileInputs` asks the method's `request<Name>Upload` Server Action for an upload grant with the file's name, type and size — never its bytes — then sends the `File` itself with the SDK's `uploadWithGrant`, imported from the browser-safe `@pipelex/sdk/upload`, and writes the grant's `pipelex-storage://` reference into the form only once storage has answered the `PUT`. The grant is a presigned, create-only `PUT` for one object, signed for exactly the declared type and size, so storage refuses anything else; it expires within minutes and is a bearer capability, so it is never logged. The bytes cross neither this app's server nor the API gateway, which is why a list of files can hold as many as the method takes: nothing caps a submission but the platform's limit on one file. The field stays busy from the grant request to the write. The SDK bounds each upload by itself, with a limit that grows with the file's size, so the hook passes no signal and a stalled upload still ends in an error rather than a spinner.
- **The grant action is the upload's gate.** `grantFileUpload` (`src/lib/uploadGrant.ts`) runs `checkUploadRequest` (`src/lib/fileInputs.ts`) over the request as untrusted JSON — a type in the method's `ALLOWED_MIMES` (`src/types/<camel>Uploads.ts`), a whole, positive size no larger than `MAX_FILE_BYTES` — before calling `requestUploadGrant`. The form reads the same list: `useRunInputs` narrows each file input to it through the kernel's `narrowFileFormats`, so the dropzone's hint, its file picker and its own check offer exactly what the grant action will grant. `MAX_FILE_BYTES` is the platform's own limit, 50 MiB, and the grant route's `413` for a larger declaration stays the authority; the browser's own size check reads the same constant and only saves a round trip. A grant route answering `404` is classified as `upload_unavailable`, naming the route and `PIPELEX_BASE_URL`: **the route is served only by a deployment that has it**, so an app pointed at one without it can store no file. A failure of the upload itself happens in the browser, where `classifyUploadError` (`src/lib/errors.ts`) reads the SDK's own classes, each by its `code` — a refusal from storage (a used or expired grant, a file that no longer matches), and an upload that never got storage's verdict (too slow for the SDK's time limit, storage out of reach, storage failing, or a fault the user cannot fix by dropping the file again). Every refusal lands in the hook's `fileError`, beside the field, before any run. `useFileInputs` also takes a `prepareFile` option, a description fix for browsers that report `file.type === ""` for a valid file; a scaffolded form passes none, so such a file is refused by the grant action's type check. A `prepareFile` that throws is shown as a file error too (`buildFilePreparationError`), since the kernel does not await `onDropFile`.
- **The run action checks references, and refuses everything else by default.** It runs the shape gate, then `checkFileInputs(DESCRIPTOR, gated.inputs)` over the **gated** inputs, then hands them to `client.prepareInputs()`, which reads the method's declared signature and passes the stored references through untouched. `prepareInputs` resolves any string it does not recognise as `data:`, `http(s)://` or `pipelex-storage://` as a **local filesystem path**, reads it and uploads it, so a Server Action that waves through an unfamiliar string is an arbitrary server-side file read. `checkFileInputs` accepts a closed set, `pipelex-storage://` and `https://` (the kernel's "paste a URL instead"; no cleartext `http://`), and refuses a `data:` URL too: nothing in this app sends a file inline, and a run request carrying bytes is what outgrew the Server Action's body limit once a method took a list of files. It finds the files by walking the method's **wire descriptor**, never by an input's name and never by sniffing values for a `url` key: a name-keyed gate fails open the day the method renames that input, and a value-keyed one cannot tell a text field named `url` from a `Document` two levels down. **`prepareInputs` takes the kernel's `{concept, content}` envelope as readily as a bare value** and preserves it on output, and a typed `InputPreparationError` it throws _before any run starts_ is classified like any other SDK error, because the options closure runs inside `executeBlockingRun` / `startDurableRun`'s `try/catch`.
- **The Server Action body limit is Next's default, 1 MB, on purpose.** Every request this app makes carries a file's description or a reference, never the file, so `next.config.js` does not raise `serverActions.bodySizeLimit`, and a body that would need it is refused outright.
- **Keep Server Function logging off.** `next dev` logs each Server Function call with its arguments, which are whatever a user typed into the form, so with the default a contract's text or a candidate's details land in the dev log in full. `next.config.js` sets `logging.serverFunctions: false`; do not remove it to debug an action — log the action's name instead.
- **File/image outputs come back as a URL** — a storage URL or a base64 data URL — in the output content. On the hosted durable path the runtime returns both a non-web `url` (`pipelex-storage://…`) and a web `public_url` (a signed S3 URL that expires in minutes and is the credential to the object). The kernel's file arms ask the host's resolver about `url` first and read `public_url` only without one, and this app mounts one (the next bullet), so a hosted run's files paint through the assets route and the signed link is never what the browser fetches. It is still in the payload the client receives — the kernel's JSON view renders the receipt verbatim — so what the resolver removes is the _sink_, not the credential. **The result view applies no URL policy of its own**: the kernel judges every URL it acts on through its exported `viewableUrl` and hands each sink the normalised string it judged, frames a document only over `http(s):` or a path the host's resolver produced, and renders an image in a text result's Markdown as a link instead of loading it. `src/components/ResultEnv.test.tsx` pins that at this app's composition, so a kernel release that loosens the gate, a resolver that echoes a payload path back, or a `proseImages="load"` on the provider fails the suite. The one channel outside that gate is an HTML result, whose markup the kernel frames under a content policy admitting `https:` images, so a remote image in model-written HTML loads on paint; `docs/input-form.md` says what a project can do about it.
- **A `pipelex-storage://` reference on its own resolves nowhere in a browser**, and the kernel's seam for exchanging one is `<ResultEnvProvider>`, which `src/components/ResultEnv.tsx` mounts once in the root layout — one provider high in the tree, never a prop threaded through `<RunResult>`. Its `resolveUrl` is `assetPath` (`src/lib/storageAsset.ts`), a pure, synchronous rewrite onto `/api/assets/…`, because the kernel's seam is synchronous by design: a resolver that rewrites has no round trip to make. `src/app/api/assets/[...path]/route.ts` streams that path through the SDK's `fetchArtifact` on the server, under `buildAssetHeaders` (`src/lib/assetHeaders.ts`) — `nosniff`; a sandboxing CSP on a document-capable type (SVG, HTML, XML), never on a PDF, whose viewer it would break; `inline` only for what the kernel previews; private caching. It is not a redirect and not an open proxy: the only input is a storage path, and a reference the key cannot see answers `404` like one that names nothing. It needs a platform serving `POST /v1/resolve-storage-url/bulk`, the route `fetchArtifact` mints through; a deployment without it answers `502` naming the route, even though the single-reference route beside it may work. Its `resolveShareUrl` is the Server Action in `src/actions/shareUrl.ts`, which mints a presigned link per click through `resolveStorageUrl` — a same-origin path is useless pasted elsewhere — and answers `undefined` on any failure, the kernel's contract for "copy the display URL instead". The key never leaves `getPipelexClient()`. A plain-http store link is fetched only when `PIPELEX_BASE_URL` itself is plain http (the local compose stack); `allowPlainHttpArtifacts` in `serverEnv.ts` decides. **The route, the share action and the grant actions are unauthenticated, deliberately and only for a single-tenant app**: the first two resolve any reference with the deployment's one API key, and a grant action lets anyone who can reach the app store a file in the deployment's organisation, so a deployment serving more than one person has to close `mayRead` in the route and ask the same question in the share and grant actions — the route's `404` hides _which_ objects exist, and that seam decides _who_ may read one.
...
The template-only files are the `make create` gesture — `scripts/create.mts`, `scripts/lib/create.mts` and its test, and `docs/create.md` — the tile e2e with its fixture, `e2e/resultTile.spec.ts` and `e2e/fixtures/generate-image`, and `docs/chrome-lineage.md`, which governs the template's relation to the gallery; the bootstrap's `REMOVALS` lists them, and its `package.json` transform drops the `create` script. **A passage of a file a project keeps that only makes sense in the template sits between two marker lines** — the words `template-only` followed by `:begin`, then by `:end`, in whatever comment the file uses — in the Makefile, this file, `AGENTS.md`, `docs/ci.md` and `docs/input-form.md`, and the bootstrap removes it with its markers. Keep each marker on a line of its own, never inside a table, and never spell a marker out anywhere else in those files: the bootstrap matches the words on any line. A test in `bootstrap.test.mjs` fails when a file a project keeps names the create gesture, or any file `REMOVALS` takes away, outside such a passage.
...
- **`home.spec.ts` is offline** and runs with no key: the page renders its title from `src/site.ts`, and either the empty state or a form, and it hydrates with no mismatch reaching the console. It holds whether or not methods have been added.
- **Wait for `html[data-hydrated]` before a script acts on the page.** `<HydrationMark>`, mounted in the root layout, sets it once React has hydrated the page. Before that, the controls are server-rendered markup with no handlers, so a file given to `setInputFiles` never reaches the handler that uploads it, and a screenshot raises a hydration mismatch of its own making: Playwright hides the caret by writing `caret-color: transparent` into every input's inline style, and React then finds an attribute it did not render. So call `page.waitForSelector("html[data-hydrated]")` after `page.goto()` and before the first screenshot, `fill` or `setInputFiles`, or pass `caret: "initial"` to a screenshot that must come earlier. **A "1 Issue" badge whose hydration diff lists an input's `style`, often with `caret-color: transparent`, comes from the script, not from the app**, and the fix is the wait, never `suppressHydrationWarning`. The mark is exact while the page hydrates in one commit; an app that adds a `loading.tsx`, a `<Suspense>` boundary or a `dynamic()` import hydrates that boundary later, and should set its own mark inside it.
<!-- template-only:begin -->
...
| `make serve`              | Start the dev server in the background, prove the page answers, print its URL (one verdict line)            |
...
**After editing anything under `methods/`, run `npm run codegen`.** `make check` compares each generated tree against a hash of the source files it was projected from, so a source edit without a regeneration fails with "Run `npm run codegen` to regenerate." rather than shipping types that quietly lie. Regeneration needs `PIPELEX_API_KEY`. Commit the regenerated tree in the same commit as the source edit. The same applies to a `methods/<name>/method.json` manifest: bumping its tag is a source edit.
...
- **No per-input validation beside the gate** — one `gateRunInputs` call per action, and no client-side twin of it. A check the contract genuinely cannot express (the file-reference scheme) runs _after_ the gate, over its output, and a file's type and size are the grant action's, reading the shared `MAX_FILE_BYTES`.
