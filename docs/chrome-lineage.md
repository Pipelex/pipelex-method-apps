...
- **The codegen kit**: the CLI entries `scripts/codegen.mts`, `scripts/codegen-check.mts`, `scripts/codegen-verify.mts` and `scripts/add-method.mts`, and `scripts/lib/api.mts`, `scripts/lib/generate.mts` and `scripts/lib/verify.mts` with their tests and the scaffold's test fixtures.
- **The configuration**: the TypeScript, Vitest, ESLint, Prettier, PostCSS and Playwright configurations, the Husky hook, the CI workflows, `.gitattributes`, `.gitignore`, `.worktreeinclude`, `.vscode/settings.json` and `LICENSE`.
