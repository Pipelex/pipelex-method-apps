...
- **The Makefile.** Besides its comments and help text, which say the keyed targets need the api-dev base URL for the time being, the `add-method` target reads only the variables given on the `make` command line and passes their values to the script exactly as typed, through the `shq`, `opt`, `flag` and `require` helpers. The gallery's target tests `METHOD` inside a shell string, where a value holding `$(…)` is executed.
